...
type BurstColorStops = {
  center: string
  middle: string
  edge: string
}

const BURST_COLOR_STOPS: Record<string, BurstColorStops> = {
  'burst-amber':   { center: '#F6C56B', middle: '#B45A16', edge: '#130804' },
  'sunburst':      { center: '#FFD35A', middle: '#A13F12', edge: '#090504' },
  'burst-cherry':  { center: '#F8B85A', middle: '#A51E22', edge: '#120506' },
  'burst-vintage': { center: '#E6A044', middle: '#713315', edge: '#080403' },
}
...
const BODY_BURST_FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    vec3 burstColor = mix(uEdgeColor, uMiddleColor, middleBlend);
    burstColor = mix(burstColor, uCenterColor, centerBlend);
...
function makeBodyBurstMaterial(mesh: THREE.Mesh, colors: ReturnType<typeof makeColors>, sourceMaterial: THREE.Material) {
  ...
  if (!bounds) {
    const fallback = new THREE.MeshStandardMaterial({ name: sourceMaterial.name || 'BodyMaterial' })
    applySolidBodyMaterial(fallback, colors)
    return fallback
  }
