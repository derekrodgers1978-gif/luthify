...
export default function GuitarCanvas() {
  ...
  const [webGLUnsupported, setWebGLUnsupported] = useState<boolean | null>(null)

  useEffect(() => {
    const testCanvas = document.createElement('canvas')
    const gl = testCanvas.getContext('webgl2') || testCanvas.getContext('webgl')
    if (!gl) {
      // Set an error state — do not render anything
  ...
  if (webGLUnsupported === true) {
    return (
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
        <div style={{ fontSize: 22, color: '#8a8790' }}>WebGL not available</div>
        <div style={{ fontSize: 13, color: '#5a5760' }}>Open in a browser with GPU acceleration enabled</div>
      </div>
    )
  }

  if (webGLUnsupported !== false) return null
